...
sagemaker_process_op = components.load_component_from_url("https://raw.githubusercontent.com/kubeflow/pipelines/master/components/aws/sagemaker/process/component.yaml")
...
def mnist_classification(role_arn="", bucket_name=""):
    ...
    train_image = "438346466558.dkr.ecr.eu-west-1.amazonaws.com/kmeans:1"

    # Training input and output location based on bucket name
    hpo_channels = [
        training_input("train", f"s3://{bucket_name}/lab1/train_data"),
        training_input("test", f"s3://{bucket_name}/lab1/test_data"),
    ]
    train_channels = [
        training_input("train", f"s3://{bucket_name}/lab1/train_data")
    ]
    train_output_location = f"s3://{bucket_name}/lab1/output"

    process = sagemaker_process_op(
        role=role_arn,
        region=region,
        image="763104351884.dkr.ecr.eu-west-1.amazonaws.com/pytorch-training:1.5.0-cpu-py36-ubuntu16.04",
        ...
        input_config=[
            processing_input(
                "mnist_tar",
                f"s3://{bucket_name}/lab1/source_data/mnist.pkl.gz",
                "/opt/ml/processing/input",
            ),
            processing_input(
                "source_code",
                f"s3://{bucket_name}/lab1/processing_code/kmeans_preprocessing.py",
    ...
    sagemaker_batch_transform_op(
        ...
        input_location=f"s3://{bucket_name}/lab1/input",
        content_type="text/csv",
        split_type="Line",
        output_location=f"s3://{bucket_name}/lab1/output",
    )
